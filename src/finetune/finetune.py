"""
Fine-Tune StarCoder on LiquidHaskell programs
"""
import argparse
import os
...
class PrintInitialStateCallback(TrainerCallback):
    def on_train_begin(
        ...
        **kwargs,
        ...
        print(f"Total Num Epochs = {state.num_train_epochs:,}")
...
def get_args():
    ...
    parser.add_argument("--model_dir", type=str, default="/tmp3/gsakkas")
    parser.add_argument("--output_dir", type=str, default="/tmp3/gsakkas/checkpoints")
    parser.add_argument("--log_dir", type=str, default="/tmp3/gsakkas/hf_logs")
...
class ConstantLengthDataset(IterableDataset):
    ...
    def __init__(
        ...
        args_eos_token_id=49152
    ):
        self.tokenizer = tokenizer
        self.concat_token_id = tokenizer.eos_token_id if tokenizer.eos_token_id is not None else args_eos_token_id
...
def run_training(args, train_data, val_data):
    ...
    training_args = TrainingArguments(
        ...
        num_train_epochs=5 if args.max_steps > 0 else args.num_epochs,
    ...
    trainer = Trainer(model=model, args=training_args, train_dataset=train_data, eval_dataset=val_data, callbacks=[PrintInitialStateCallback, SavePeftModelCallback, LoadBestPeftModelCallback])
    print("Training...")
    trainer.train()

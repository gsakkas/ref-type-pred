...
def download_contents(blob_id, src_encoding):
    ...
    with so.open(s3_url, "rb", compression=".gz", transport_params={"client": s3}) as fin:
...
ds = load_dataset("bigcode/the-stack-v2-dedup", "Haskell", cache_dir="/tmp3/gsakkas/huggingface", split="train")
...
for dss in ds:
    content = download_contents(dss["blob_id"], dss["src_encoding"])
    if "{-@" in content:
        ground_truth = re.findall(r"{-@\s*?\S*?\s*?::[\s\S]*?@-}", content)
        total += 1
        if ground_truth:
            count += 1
            all_lh_types += len(ground_truth)
            filename = dss['repo_name'] + "/" + dss['path']
            # if '\\' in filename:
            ...
            for lh_type in ground_truth:
                ...
                bad_prog = starcoder_prompt + content
                ...
                if lh_type not in unq_lh_types_also_in_tutorial and not (sim_ratio < 0.7):
...
ds.save_to_disk("./benchmarks/lh_training_set_from_the_stack_v2.hf")

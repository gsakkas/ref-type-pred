...
USE_HASKEL_TYPES = False # i.e. haskell types in programs, but no tests
USE_HASKEL_TYPES_AND_TESTS = True # i.e. haskell types and tests in programs
...
with open(f"lh_test_set_final{file_suffix}.jsonl", "w", encoding="utf-8") as out_file:
    for sample_dict in json_dataset:
        out_file.write(json.dumps(sample_dict) + '\n')

with open(f"lh_test_set_labels_final{file_suffix}.jsonl", "w", encoding="utf-8") as out_file:
